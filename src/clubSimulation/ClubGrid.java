...
public class ClubGrid {
	...
	public GridBlock enterClub(PeopleLocation myLocation) throws InterruptedException  {
      ...
      synchronized(entrance) //acquire the lock and block other threads from entering. Synhronizing on entrance object allows only one thread to check for entrance at any given time
      {
         
         while(counter.overCapacity() || !entrance.get(myLocation.getID())) //entrance rule not met wait. 
         {
            System.out.println("Thread "+myLocation.getID() + " is waiting to enter ");
            entrance.wait();
         }
	...
	public GridBlock move(GridBlock currentBlock,int step_x, int step_y,PeopleLocation myLocation) throws InterruptedException {  //try to move in 
		...
		if (!newBlock.get(myLocation.getID())) return currentBlock; //stay where you are because the new grid is occupied
			
		currentBlock.release(); //must release current block
		myLocation.setLocation(newBlock);
      
      
      if((entrance.getX()==currentBlock.getX())&&(entrance.getY()==currentBlock.getY()))
      {
         synchronized(entrance) 
         {
			   entrance.notify(); //notify waiting threads if moved away from the entrance
         }
      }
      ...
      public GridBlock moveB(GridBlock currentBlock,int step_x, int step_y,PeopleLocation myLocation) throws InterruptedException {  //DEFINES BARMAN MOVEMENTS THAT OVERRIDE THE NORMAL PERSON NAVIGATION RULES
	...
	public  void leaveClub(GridBlock currentBlock,PeopleLocation myLocation){
         ...
         synchronized(entrance) //acquire the lock and block other threads and notify all threads to try to enter
         {
			   entrance.notify();
         }
