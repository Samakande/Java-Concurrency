...
public class Clubgoer extends Thread {
   ...
   public static Object startObj = new Object();
   //public static Barman andrea;
   public static boolean pause =false; //shared variable toggled by the user pause button to indicate to threads to pause
   ...
   private void checkPause() {
       if (pause) {
           synchronized (club) { //execution of wait is within schronized block to avoid deadlocks
               try {
                   club.wait();
	...
	private void startSim() {
           synchronized (startObj) { // the startSim is called at the begging of the run method it will immediatelt halt all threads from running pending a user input
               try {
                   startObj.wait();
               } catch (InterruptedException e) {
                   e.printStackTrace();
